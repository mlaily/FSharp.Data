...
let ``InferColumnTypes shall infer empty string as Double``() =
  ...
  let types = csv.InferColumnTypes(2,[|""|], StructuralTypes.InferenceMode.InferTypesFromValuesOnly, System.Globalization.CultureInfo.GetCultureInfo(""), null, false, false, StructuralInference.defaultUnitsOfMeasureProvider)
  let expected = "Double"
