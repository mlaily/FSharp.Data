...
//    /// Converts the input (expected type varies) into the type specified by field.
//    let convert (value: Expr) =
//        let convertJsonValueOpt jsonValueOpt =
//            getConversionQuotation cultureStr field.InferedType jsonValueOpt
...
//                    (field.RuntimeType)
//                    (<@ (%varExpr).Path @>, convertJsonValueOpt <@ (%varExpr).JsonOpt @>, <@ (%varExpr).JsonOpt @>))
//        | TypeWrapper.None, false ->
//            wrapInLetIfNeeded value (fun (varExpr: Expr<IJsonDocument2>) ->
//                typeof<JsonRuntime2>?(nameof (JsonRuntime2.GetNonOptionalValue))
//                    (field.RuntimeType)
//                    (<@ (%varExpr).Path() @>, convertJsonValueOpt <@ Some (%varExpr).JsonValue @>, <@ Some (%varExpr).JsonValue @>))
//        | TypeWrapper.Option, true -> convertJsonValueOpt <@ (%%value: JsonValue2 option) @>
//        | TypeWrapper.Option, false ->
//            //TODO: not covered in tests
//            convertJsonValueOpt <@ Some (%%value: IJsonDocument2).JsonValue @>
//        | TypeWrapper.Nullable, true ->
//            //TODO: not covered in tests
//            typeof<TextRuntime>?(nameof (TextRuntime.OptionToNullable))
//                (field.RuntimeType)
//                (convertJsonValueOpt <@ (%%value: JsonValue2 option) @>)
