...
[<Extension>]
type JsonExtensions2 =

    /// Get a sequence of key-value pairs representing the properties of an object
    [<Extension>]
    static member Properties(x: JsonValue2) =
        match x with
        | JsonValue2.Record properties -> properties
        | _ -> [||]
    ...
    [<Extension>]
    static member inline Item(x, propertyName) =
        JsonExtensions2.GetProperty(x, propertyName)

    /// Get all the elements of a JSON value.
    /// Returns an empty array if the value is not a JSON array.
    [<Extension>]
    static member AsArray(x: JsonValue2) =
        match x with
    ...
    static member inline GetEnumerator(x) =
        JsonExtensions2.AsArray(x).GetEnumerator()

    /// Try to get the value at the specified index, if the value is a JSON array.
    [<Extension>]
    static member inline Item(x, index) = JsonExtensions2.AsArray(x).[index]
    ...
    static member AsString(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsString false cultureInfo x with
        | Some s -> s
        | _ ->
    ...
    static member AsInteger(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsInteger cultureInfo x with
        | Some i -> i
        | _ ->
    ...
    static member AsInteger64(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsInteger64 cultureInfo x with
        | Some i -> i
        | _ ->
    ...
    static member AsDecimal(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsDecimal cultureInfo x with
        | Some d -> d
        | _ ->
    ...
    static member AsFloat(x, [<Optional>] ?cultureInfo, [<Optional>] ?missingValues) =
        ...
        match JsonConversions2.AsFloat missingValues false cultureInfo x with
        | Some f -> f
        | _ ->
    ...
    static member AsBoolean(x) =
        match JsonConversions2.AsBoolean x with
        | Some b -> b
        | _ ->
    ...
    static member AsDateTime(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsDateTime cultureInfo x with
        | Some d -> d
        | _ ->
    ...
    static member AsDateTimeOffset(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsDateTimeOffset cultureInfo x with
        | Some d -> d
        | _ ->
            failwithf "Not a datetime offset: %s"
            <| x.ToString(JsonSaveOptions2.DisableFormatting)
    ...
    static member AsTimeSpan(x, [<Optional>] ?cultureInfo) =
        ...
        match JsonConversions2.AsTimeSpan cultureInfo x with
        | Some t -> t
        | _ ->
    ...
    static member AsGuid(x) =
        match JsonConversions2.AsGuid x with
        | Some g -> g
        | _ ->
    ...
    static member InnerText(x) =
        match JsonConversions2.AsString false CultureInfo.InvariantCulture x with
        | Some str -> str
        | None ->
            JsonExtensions2.AsArray(x)
            |> Array.map (fun e -> JsonExtensions2.InnerText(e))
            |> String.Concat
...
module JsonExtensions =
    ...
    let (?) (jsonObject: JsonValue2) propertyName = jsonObject.GetProperty(propertyName)

    type JsonValue2 with
        member x.Properties =
            match x with
            | JsonValue2.Record properties -> properties
            | _ -> [||]
...
module Options =
    ...
    type JsonValue with
        ...
        /// Get the guid value of an element (assuming that the value is a guid)
        member x.AsGuid() = JsonConversions.AsGuid x
    ...
    type JsonValueOptionExtensions() =
        ...
        [<Extension>]
        static member inline Item(x, propertyName) =
            JsonValueOptionExtensions.TryGetProperty(x, propertyName)
        ...
        [<Extension>]
        static member inline GetEnumerator(x) =
            JsonValueOptionExtensions.AsArray(x).GetEnumerator()

        /// Try to get the value at the specified index, if the value is a JSON array.
        [<Extension>]
        static member inline Item(x, index) =
            JsonValueOptionExtensions.AsArray(x).[index]
        ...
        /// Get the guid value of an element (assuming that the value is a guid)
        [<Extension>]
        static member AsGuid(x) = x |> Option.bind JsonConversions.AsGuid
        ...
        static member InnerText(x) =
            ...
            | Some str -> str
            | None ->
                JsonValueOptionExtensions.AsArray(x)
                |> Array.map (fun e -> e.InnerText)
                |> String.Concat
