...
type JsonValue2 =
    ...
    | Number of decimal
    | Float of float
    | Record of properties: (string * JsonValue2)[]
    | Array of elements: JsonValue2[]
    | Boolean of bool
    | Null
    ...
    member x.WriteTo(w: TextWriter, saveOptions) =
        ...
        let newLine =
            if saveOptions = JsonSaveOptions2.None then
        ...
        let propSep = if saveOptions = JsonSaveOptions2.None then "\": " else "\":"
        ...
        let rec serialize indentation =
            ...
            | String s ->
                w.Write "\""
                JsonValue2.JsonStringEncodeTo w s
                w.Write "\""
            | Record properties ->
                w.Write "{"
                ...
                for i = 0 to properties.Length - 1 do
                    ...
                    w.Write "\""
                    JsonValue2.JsonStringEncodeTo w k
                    w.Write propSep
    ...
    override x.ToString() = x.ToString(JsonSaveOptions2.None)
...
type private JsonParser(jsonText: string) =
    ...
    let rec parseValue cont =
        ...
        match s.[i] with
        | '"' -> cont (JsonValue2.String(parseString ()))
        | '-' -> cont (parseNum ())
        | c when Char.IsDigit(c) -> cont (parseNum ())
        | '{' -> parseObject cont
        | '[' -> parseArray cont
        | 't' -> cont (parseLiteral ("true", JsonValue2.Boolean true))
        | 'f' -> cont (parseLiteral ("false", JsonValue2.Boolean false))
        | 'n' -> cont (parseLiteral ("null", JsonValue2.Null))
        | _ -> throw ()
    ...
    and parseObject cont =
        ...
        let parseObjectEnd () =
            ...
            i <- i + 1
            let res = pairs.ToArray() |> JsonValue2.Record
            cont res
    ...
    and parseArray cont =
        ...
        let parseArrayEnd () =
            ...
            i <- i + 1
            let res = vals.ToArray() |> JsonValue2.Array
            cont res
...
type JsonValue2 with
    ...
    static member Load(uri: string, [<Optional>] ?encoding) =
        JsonValue2.AsyncLoad(uri, ?encoding = encoding)
    ...
    member private x.PrepareRequest(httpMethod, headers) =
        ...
        TextRequest(x.ToString(JsonSaveOptions2.DisableFormatting)), headers, httpMethod
